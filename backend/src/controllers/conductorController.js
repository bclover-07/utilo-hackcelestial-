...
export const conductorController = {
  stream: async (req, res) => {
    ...
    try {
      const plan = await conductor.createPlan(req.user, req.body, { onProgress: progress => send({ type: "progress", ...progress }) });
  ...
  create: async (req, res) => res.status(201).json(await conductor.createPlan(req.user, req.body)),
  ...
  replan: async (req, res) => res.json(await conductor.replan(req.user, id.parse(req.params.id), req.body)),
